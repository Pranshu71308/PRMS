...
namespace Amnex_Project_Resource_Mapping_System.Controllers
{
    public class DepartmentsController : Controller
    {
        ...
        public IActionResult Deletedepartment(Department model)
        {
            ...
            try
            {
                var sql1 = "SELECT public.checkisdepartmentdeleteable(@in_department_id)";
                bool isDeleteable;
                using (var cmd1 = new NpgsqlCommand(sql1, _connection))
                {
                ...
                if (isDeleteable)
                {
                    ...
                    using (var cmd2 = new NpgsqlCommand(sql, _connection))
                    {
                        ...
                        cmd2.Parameters.AddWithValue("in_modify_by", Convert.ToInt32(uid));
                ...
                else
                {
                    ...
                    throw new InvalidOperationException("Department is not deleteable.");
                }
            }
            catch (Exception ex)
            {
